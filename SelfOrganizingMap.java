...
public class SelfOrganizingMap extends AbstractClusterer
    ...
    private double [][] m_cluster2instances;
    private double []m_pi;    
    private double oldtotalllk;
    private List [] m_listcluster;
    private double[][] m_kl;
    
    private static List<DenseMatrix64F> x_instances;
    private double [][] m_cluster2instances2;
    private static List<DenseMatrix64F> m_centers;
    private double helpme;
    ...
    public void buildClusterer(Instances data) throws Exception {
		...
		// the winner neuron
		int winningNeuron;
		nearestpoint = new int[2];

		initmatrixs(); // lzx initialize every matrix
		initmyclusters(); // initialize clusters
		...
		for (int epoch = 1; epoch <= m_convergenceEpochs + m_orderingEpochs; epoch++) {
			...
			for (int instance = 0; instance < m_instances.numInstances(); instance++) {
				...
				winningNeuron = bestfitdistribution(instance);
				// update the weights
				for (int neuron = 0; neuron < m_clusters.numInstances(); neuron++) {
					updateWeights(neuron, winningNeuron, instance,
							learningRate, effectiveWidth, epoch);
    ...
    {
    	//lzx init every cluster to instances list, and every cluster's distribution
        m_cluster2instances= new double[m_clusters.numInstances()][m_instances.numInstances()];       
        m_cluster2instances2= new double[m_clusters.numInstances()][m_instances.numInstances()];                    
        m_kl=new double[m_clusters.numInstances()][m_clusters.numInstances()];            
        m_listcluster=new ArrayList[m_clusters.numInstances()];
    }
    /*initialize clusters through enforcement
     * 
     */
    public void initmyclusters(){
		// calculate m_pi and N_k		
		int d = m_instances.numInstances() / m_clusters.numInstances();
		int lastd = m_instances.numInstances() - d
				* (m_clusters.numInstances() - 1);				
		for (int cluster = 0; cluster < (m_clusters.numInstances()-1); cluster++) {
			m_listcluster[cluster]=new ArrayList<Integer>();
			for (int instance = cluster * d; instance < d
					* (cluster+ 1); instance++) {
				m_cluster2instances[cluster][instance] = 1;
				m_listcluster[cluster].add(instance);
			}
		}
		{
			m_listcluster[m_clusters.numInstances() - 1]=new ArrayList<Integer>();
			for (int instance = d * (m_clusters.numInstances() - 1); instance < m_instances
					.numInstances(); instance++) {
				m_cluster2instances[m_clusters.numInstances() - 1][instance] = 1;
				m_listcluster[m_clusters.numInstances() - 1].add(instance);
			}
		}		
		//initilize kl
		for (int cluster = 0; cluster < m_clusters.numInstances(); cluster++) {
			for (int cluster2 = 0; cluster2 < m_clusters.numInstances(); cluster2++) {
				double distance = 0;
				for (int instance = 0; instance < m_instances.numInstances(); instance++) {
					double dis = Math
							.sqrt(m_cluster2instances[cluster][instance])
							- Math.sqrt(m_cluster2instances[cluster2][instance]);
					distance += dis * dis;
				}
				m_kl[cluster][cluster2] = distance / 2;
			}
		}			
    }
    public void Hellinger2(){
    	for(int cluster=0;cluster<m_clusters.numInstances();cluster++){
    		for(int cluster2=0;cluster2<m_clusters.numInstances();cluster2++){
    			double distance=0;
    			for(int instance=0;instance<m_instances.numInstances();instance++){
    				double d=Math.sqrt(m_cluster2instances[cluster][instance])-Math.sqrt(m_cluster2instances[cluster2][instance]);
    				distance+=d*d;
    			}    			
    			m_kl[cluster][cluster2]=distance/2;
    			//System.out.println(cluster+" "+cluster2+" "+m_kl[cluster][cluster2]);
    		}
    	}
    ...
    public void assigncluster(){
    	for(int cluster=0;cluster<m_clusters.numInstances();cluster++){
    		m_listcluster[cluster]=null;
    		m_listcluster[cluster]=new ArrayList<Integer>();
    	}
    	for(int instance=0;instance<m_instances.numInstances();instance++){
    		int winningcluster=0;
    		double a=0;
    		for(int cluster=0;cluster<m_clusters.numInstances();cluster++){
    			if(a<m_cluster2instances[cluster][instance]){
    				a=m_cluster2instances[cluster][instance];
    				winningcluster=cluster;
    			}
    		}
    		m_listcluster[winningcluster].add(instance);    		
    	}
    	
    }
    
    private int bestfitdistribution(int instance){			
    	//double a=Math.sqrt(m_clusters.numAttributes());
    	double a=m_clusters.numAttributes();
		for (int cluster = 0; cluster < m_clusters.numInstances(); cluster++) {
			double nearest = Double.MAX_VALUE;
			int nearpoint = 0;
			for (int ins = 0; ins < m_listcluster[cluster].size(); ins++) {
				int i = (Integer) m_listcluster[cluster].get(ins);
				if (instance == i)
					continue;
				double dis = 0;
				for (int attribute = 0; attribute < m_clusters.numAttributes(); attribute++) {
					dis += (m_instances.get(i).value(attribute) - m_instances
							.get(instance).value(attribute))
							* (m_instances.get(i).value(attribute) - m_instances
									.get(instance).value(attribute));
				}
				dis = Math.sqrt(dis);
				if (dis < nearest) {
			...
			nearestpoint[cluster]=nearpoint;
			m_cluster2instances[cluster][instance] = 1.0/(nearest);
		}
		int winningNeuron = 0;
		double winningdistance = 0;
		double totalp = 0;
		for (int cluster = 0; cluster < m_clusters.numInstances(); cluster++) {
			totalp += m_cluster2instances[cluster][instance];
			if (winningdistance < m_cluster2instances[cluster][instance]) {
				winningdistance = m_cluster2instances[cluster][instance];
				winningNeuron = cluster;
			}
		}
     ...
     */
    private void updateWeights(int updateCluster, int winningCluster, int instance, double learningRate, double effWidth,int iteration) {        
    	
    	double diff;
    	double e;
    	if(iteration<m_orderingEpochs)
    		e=Math.exp(-iteration/m_orderingEpochs);
    	else 
    		e=0.001;
    	double distance=Math.exp(-m_kl[updateCluster][winningCluster]/e);
    	double h=distance;
    	for (int j = 0; j < m_clusters.numAttributes(); j++) {
    	...
    	helpme=m_kl[updateCluster][winningCluster];
    }
